...
class Connection(object):
    def __init__(self,linear_serial, rotary_serial):
        self.con1 = serial.Serial(None, baudrate=9600, timeout = 0, writeTimeout = 0) #linear stage
        self.con2 = serial.Serial(None, baudrate=9600, timeout = 0, writeTimeout = 0) #rotary head
        self.connection_attempts = 0
        self.rotary_serial = rotary_serial
        self.linear_serial = linear_serial
    ...
    def connect_to_ports(self):
        ...
        for port in xrange(0,9):
            try:
	            ...
	            if not self.con1.isOpen(): self.con1.open()
                    sleep(2)
    ...
    def assign_serial_num(self, sn, x_port, y_port, con):
        if sn == self.rotary_serial and y_port is False:
            y_port = True
            self.motor_y = Motor(con)
            
        elif sn == self.linear_serial  and x_port is False:
            x_port = True
...
class Motor(object):
    ...
    def __init__(self, serial_con):
        ...
        self._cm_per_rev = 0.2 #for linear head
        '''2mm per 1 full revolution '''
    ...
    def main(self, acceleration, max_vel, init_vel, cm_per_rev, isLinear):
	...
	self.isLinear = isLinear
	    ...
	    self._set_var('LM', 2)# decel ramp, stops all motion that dir
            self._set_var('S1','3,1,0') # limit switch for home, second paramater specifies the activ estate
            self._set_var('S2','2,1,0')# limit switch for farthest end
        else:
	
	    self._set_var('LM', 2)# decel ramp, stops all motion that dir
            self._set_var('S1','3,0,0') # limit switch for home, second paramater specifies the activ estate
...
#         self.con.readlines()
    ...
    def _calculate_pos(self, steps):
        ...
        X steps * 1 rev/Y steps * 2 mm/1 rev '''
        CurrentPos =  steps * (1.0/(self._steps_per_rev[str(self.MicroStep)])) * self._cm_per_rev
    ...
    def _calc_steps(self,linear_dist):
        '''How many steps it takes to get to a linear dist input in cm.'''
        steps = float(linear_dist)/self._cm_per_rev * float((self._steps_per_rev[str(self.MicroStep)]))
	...
	self._CurrentStep = self._get_current_step()
        ...
        if self.isLinear: print "New Pos: %s cm" %self.CurrentPos
	else: print "New Pos: %s deg" %self.CurrentPos
    ...
    def angle_scan(self, homing = True):
        '''if homing is True, align and reset position as start, or else return the angle measured. Only use this function for rotary head'''
        flag = True
        while flag:
            movement = raw_input('move left or right? press o for left, p for right, default step size is 1 degree/3600 arcsec: [o/p] [stepsize in arcsec] press Enter if you are happy. : ')
            if movement == 'p':self.move_rel(1)
            elif movement == 'o': self.move_rel(-1)
            elif movement == '': flag = False
	    elif movement[0] == 'p' or movement[0] == 'o' and len(movement) > 1:
		try: 
		    deg = float(eval(movement[2:]))/3600.
		    if movement[0] == 'p': self.move_rel(1*deg)
		    else: self.move_rel(-1*deg)
		except ValueError: print("Second parameter needs to be a number! Usage: [o/p] [stepsize in arcsec]")
            else: print('You did not enter the correct input')
        if homing:
            self.set_pos_as_start()
	else:
	    print('Measured angle is ' + str(self.CurrentPos) + 'deg')
